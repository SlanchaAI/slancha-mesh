...
_VIDEO_CLEANUP_RETRY_S = 30.0
_VIDEO_CLEANUP_SHUTDOWN_S = _VIDEO_DELETE_DEADLINE_S + 10.0
...
def create_router_app(
    ...
    *,
    ...
    async def _lifespan(app: FastAPI):
        ...
        if video_job_db_path is not None:
            cleanup_task = asyncio.create_task(_video_cleanup_loop(cleanup_stop))
        try:
            yield
        finally:
            if cleanup_task is not None:
                ...
                try:
                    await asyncio.wait_for(
                        cleanup_task,
                        timeout=_VIDEO_CLEANUP_SHUTDOWN_S,
                    )
                except TimeoutError:
                    _log.warning("[router] video cleanup shutdown timed out")
    ...
    async def _delete_video_cleanup_owner(
        *,
        owner_origin: str,
        upstream_job_id: str,
    ) -> bool:
        """Bound one exact-owner cleanup attempt; accept only the vLLM-Omni contract."""
        ...
        try:
            async with asyncio.timeout(_VIDEO_DELETE_DEADLINE_S):
                async with client.stream(
                    "DELETE",
                    f"{owner_origin}/v1/videos/{upstream_job_id}",
                    headers=_multipart_upstream_headers(),
                    follow_redirects=False,
                ) as upstream:
        ...
        return bool(
            ...
            and payload.get("id") == upstream_job_id
            and payload.get("deleted") is True
            and payload.get("object") == "video.deleted"
        )

    async def _video_cleanup_loop(stop: asyncio.Event) -> None:
        """Drain expired/orphaned owner rows; producer is VideoJobStore."""

        while not stop.is_set():
            claim: VideoDeleteClaim | None = None
            ...
            try:
                ...
                if claim is not None:
                    if claim.job.protocol_id != VIDEO_JOB_PROTOCOL.protocol_id:
                        await run_in_threadpool(
                            store.quarantine_cleanup,
                            claim.job.public_id,
                            claim.token,
                        )
                        continue
                    owner = _recorded_video_owner(claim.job, _snapshot())
                    if owner is not None and await _delete_video_cleanup_owner(
                        owner_origin=claim.job.owner_origin,
                        upstream_job_id=claim.job.upstream_job_id,
                    ):
                        await run_in_threadpool(
                ...
                _log.warning(
                    "[router] video cleanup deferred: %s",
                    type(exc).__name__,
                )
            if claim is None:
                try:
                    await asyncio.wait_for(
                        stop.wait(),
                        timeout=_VIDEO_CLEANUP_RETRY_S,
                    )
                except TimeoutError:
                    pass
    ...
    async def create_video_job(
        ...
        for part in parts:
            if part.filename is not None:
                continue
            scalar_counts[part.name] = scalar_counts.get(part.name, 0) + 1
        duplicate_scalar = next(
            (name for name, count in scalar_counts.items() if count > 1),
            None,
        )
        if duplicate_scalar is not None:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=(
                    f"duplicate multipart scalar field {duplicate_scalar!r}; "
                    "each scalar may appear at most once"
                ),
            )
        specialist_id = model_parts[0]
        snapshot = _snapshot()
        ...
        try:
            ...
            compensated = await _delete_video_cleanup_owner(
                owner_origin=owner_origin,
                upstream_job_id=upstream_id,
            )
            ...
            if not compensated:
                try:
                    cleanup_store = store or await run_in_threadpool(_video_store)
                    await run_in_threadpool(
                        cleanup_store.enqueue_cleanup,
                        protocol_id=VIDEO_JOB_PROTOCOL.protocol_id,
                        specialist_id=specialist_id,
                        owner_node_id=binding.node_id,
                        owner_origin=owner_origin,
                    ...
                    pass
            _log.warning(
                "[router] video create ownership failed owner=%s specialist=%s "
                "store=%s compensated=%s cleanup_queued=%s",
                binding.node_id[:128],
                specialist_id[:128],
                type(exc).__name__,
                compensated,
