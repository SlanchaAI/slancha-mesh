...
def test_video_create_persistence_failure_reports_bounded_orphan_once(
    ...
    assert "compensated=false" in caplog.text.lower()
    assert "cleanup_queued=false" in caplog.text.lower()
...
def test_video_create_rejects_duplicate_scalar_before_upstream(tmp_path) -> None:
    response = TestClient(
        _video_app(
            _snapshot(capabilities=[VIDEO_JOB_CAPABILITY]),
...
def test_video_create_store_failure_compensates_on_exact_owner(tmp_path, monkeypatch) -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        if request.method == "POST":
            return httpx.Response(
                200,
                json={"id": "upstream-orphan", "status": "queued", "created_at": 1},
            )
        return httpx.Response(
            200,
            json={
    ...
    assert response.status_code == 502
    assert "upstream-orphan" not in response.text
    assert calls == [
        ("POST", "/v1/videos"),
        ("DELETE", "/v1/videos/upstream-orphan"),
    ]


def test_video_create_failed_compensation_persists_hidden_cleanup_work(
    ...
    with sqlite3.connect(db_path) as connection:
        row = connection.execute(
            "SELECT upstream_job_id, cleanup_pending FROM video_jobs"
        ).fetchone()
    assert row == ("upstream-orphan", 1)


def test_video_cleanup_worker_deletes_durable_outbox_on_restart(tmp_path) -> None:
    db_path = tmp_path / "video-jobs.sqlite3"
    store = VideoJobStore(db_path)
    store.enqueue_cleanup(
        protocol_id="vllm_omni.video.jobs.v1",
        specialist_id=SPECIALIST_ID,
        owner_node_id="media-node",
        owner_origin=NODE_URL,
        upstream_job_id="upstream-orphan",
        status="queued",
    )
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        assert request.url == httpx.URL(f"{NODE_URL}/v1/videos/upstream-orphan")
        deleted.set()
        return httpx.Response(
            200,
            json={
...
def test_video_cleanup_shutdown_finishes_confirmed_inflight_delete(
    tmp_path, monkeypatch
) -> None:
    db_path = tmp_path / "video-jobs.sqlite3"
    store = VideoJobStore(db_path)
    store.enqueue_cleanup(
        ...
        owner_node_id="media-node",
        owner_origin=NODE_URL,
        upstream_job_id="upstream-inflight",
        status="queued",
    )
    response_read = threading.Event()
    finish_read = threading.Event()
    from mesh import router_app

    original_read = router_app._read_bounded_response

    async def delayed_read(*args, **kwargs):
        content = await original_read(*args, **kwargs)
        response_read.set()
        await asyncio.get_running_loop().run_in_executor(None, finish_read.wait)
        return content

    monkeypatch.setattr(router_app, "_read_bounded_response", delayed_read)

    def handler(request: httpx.Request) -> httpx.Response:
        return httpx.Response(
            200,
            json={
                "id": "upstream-inflight",
                "deleted": True,
                "object": "video.deleted",
            },
        )

    release = threading.Timer(0.1, finish_read.set)
    with TestClient(
        _video_app(_snapshot(capabilities=[VIDEO_JOB_CAPABILITY]), handler, db_path)
    ):
        assert response_read.wait(timeout=2)
        release.start()
...
def test_video_cleanup_worker_never_calls_owner_for_wrong_protocol(tmp_path) -> None:
    db_path = tmp_path / "video-jobs.sqlite3"
    store = VideoJobStore(db_path)
    store.enqueue_cleanup(
        protocol_id="vllm_omni.video.jobs.v1",
    ...
    with sqlite3.connect(db_path) as connection:
        connection.execute(
            "UPDATE video_jobs SET protocol_id = 'untrusted.protocol.v1'"
        )
    ...
    with sqlite3.connect(db_path) as connection:
        assert connection.execute(
            "SELECT cleanup_pending FROM video_jobs"
        ).fetchone()[0] == 2


def test_video_cleanup_missing_owner_does_not_starve_later_work(tmp_path) -> None:
    db_path = tmp_path / "video-jobs.sqlite3"
    store = VideoJobStore(db_path)
    store.enqueue_cleanup(
        protocol_id="vllm_omni.video.jobs.v1",
    ...
    store.enqueue_cleanup(
        ...
        owner_node_id="media-node",
        owner_origin=NODE_URL,
        upstream_job_id="ready-job",
        status="queued",
    )
    deleted = threading.Event()

    def handler(request: httpx.Request) -> httpx.Response:
        assert request.url.path == "/v1/videos/ready-job"
        deleted.set()
        return httpx.Response(
            200,
            json={"id": "ready-job", "deleted": True, "object": "video.deleted"},
        )
    ...
    with sqlite3.connect(db_path) as connection:
        rows = connection.execute(
            "SELECT upstream_job_id, delete_claim_token FROM video_jobs"
        ).fetchall()
    assert len(rows) == 1
    assert rows[0][0] == "missing-job"
