...
def test_advertise_url_swaps_host_keeps_port_and_scheme():
    out = advertise_url("http://0.0.0.0:8003", "gb10-node.tnet-example.ts.net")
    assert out == "http://gb10-node.tnet-example.ts.net:8003"
...
def test_resolution_identical_across_control_planes():
    ...
    assert ts == hs == "gb10-node.tnet-example.ts.net"
...
def test_advertise_host_alone_stays_lan_mode(monkeypatch):
    ...
    args = argparse.Namespace(key=None, tailnet=False,
                              advertise_host="compute-node",
                              control_plane=None, login_server=None)
    cfg = _tailnet_from_args(args)
    assert cfg.enabled is False
    assert cfg.advertise_host == "compute-node"
...
def test_build_daemon_lan_advertise(monkeypatch):
    ...
    cfg = TailnetConfig(enabled=False, advertise_host="compute-node")
    daemon = build_daemon(specialist_ids=[], tailnet=cfg)
    assert daemon.advertise_host == "compute-node"
